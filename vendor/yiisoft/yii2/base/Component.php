 ...
 * attached.
 *
 * 组件是实现*属性*、*事件*和*行为*特性的基类。
 *
组件提供了*事件*和*行为*特性，除了*属性*特性
 *它的父类[[\yii\base\BaseObject|BaseObject]]。
 *
 * Event是一种将自定义代码“注入”到特定位置的现有代码中的方法。例如，注释对象可以触发
 *当用户添加评论时，一个“添加”事件。我们可以编写自定义代码并将其附加到这个事件上，这样当事件发生时
 *被触发(即添加注释)，我们的自定义代码将被执行。
 *
 *一个事件由一个在定义它的类中唯一的名称来标识。事件名称区分大小写。
 *
 *一个或多个PHP回调，称为*事件处理程序*，可以附加到一个事件。你可以调用[[trigger()]] to
提出一个事件。当事件被引发时，事件处理程序将按照它们被调用的顺序自动调用
 *附呈。
 *
